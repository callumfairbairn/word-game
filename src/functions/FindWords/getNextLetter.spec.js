...
describe('getNextLetter', () => {
  ...
  it('returns the letter to the right and down of the input letter if possible', () => {
    const location = { x: 1, y: 1 }
    const direction = 'downright'
    const grid = generateGrid(generateRandomLetterList())

    expect(getNextLetter(location, direction, grid).location).toEqual({ x: 2, y: 2 })
  })

  it('returns undefined if right and down is impossible', () => {
    const location = { x: 3, y: 3 }
    const direction = 'downright'
    const grid = generateGrid(generateRandomLetterList())
  ...
  it('returns the letter to the left and down of the input letter if possible', () => {
    const location = { x: 1, y: 1 }
    const direction = 'downleft'
    const grid = generateGrid(generateRandomLetterList())

    expect(getNextLetter(location, direction, grid).location).toEqual({ x: 2, y: 0 })
  })

  it('returns undefined if left and down is impossible', () => {
    const location = { x: 3, y: 3 }
    const direction = 'downleft'
    const grid = generateGrid(generateRandomLetterList())
  ...
  it('returns the letter to the left and up of the input letter if possible', () => {
    const location = { x: 1, y: 1 }
    const direction = 'upleft'
    const grid = generateGrid(generateRandomLetterList())

    expect(getNextLetter(location, direction, grid).location).toEqual({ x: 0, y: 0 })
  })

  it('returns undefined if left and up is impossible', () => {
    const location = { x: 0, y: 0 }
    const direction = 'upleft'
    const grid = generateGrid(generateRandomLetterList())
  ...
  it('returns the letter to the right and up of the input letter if possible', () => {
    const location = { x: 1, y: 1 }
    const direction = 'upright'
    const grid = generateGrid(generateRandomLetterList())

    expect(getNextLetter(location, direction, grid).location).toEqual({ x: 0, y: 2 })
  })

  it('returns undefined if right and up is impossible', () => {
    const location = { x: 0, y: 0 }
    const direction = 'upright'
    const grid = generateGrid(generateRandomLetterList())
